...
import underMaintenanceImg from '../../images/under-maintenance.jpg'
...
const menu = [
	...
	},
	{
		path: '#', name: 'Support', icon: <TeamOutlined />, auth: ['ADMIN','SELLER'], children: [
			{ path: '/manage-caselog', name: 'User Help', icon: <UserOutlined />, auth: ['ADMIN'] },
...
class AppSidebar extends Component {
	...
	getSiteSettingData = async () => {
		try {
			...
			console.log('user_detailssssssssss');
			console.log(user_detail?.isBussinessVerified);
			this.setState({ isBusinessVerfied: user_detail?.isBussinessVerified })
		...
		return (
			...
			<Menu mode="inline" defaultSelectedKeys={[routepath]}
				...
				{menu.map((item) => {
					...
					if (item.auth.find(val => val === localStorage.getItem('role')) && this.state.isBusinessVerfied) {
          ...
          >
                        <img style={{  width: "100%", height: "80vh" }}  src={underMaintenanceImg} alt="Galinukkad" />
                </Modal>
