...
class AppHeader extends Component {
  ...
  async componentDidMount() {
    ...
    clearAllNotifications = async () =>  {
      ...
      notification.success({message: "Notification cleared successfully."});
  ...
  render() {
    ...
    const menu = (
      <Menu onClick={this.onMenuClick}>
        <Menu.Item key="setting">
          <center>
            <span   style={{ fontWeight: "bold", textTransform: "capitalize" , backgroundColor: 'white' }}>
              
             <span className="align_header">{user.isBussinessVerified ? <img src={VerifiedImg} style={{ height: "14px", width: "14px", marginTop: "-4px", marginRight: "5px" }} />  : ""}</span> 
              <span className="align_header_name">{user.username} </span>
            </span>
          </center>
        </Menu.Item>
        {/* {role === "ADMIN" && ( */}
    ...
    let notificationDropdown = (
      ...
      <Menu onClick={this.onMenuClick} >
          ...
          <a href="javascript:void(0)" class="notification_css" style={{ marginLeft: '10px' }} onClick={ this.clearAllNotifications } > Clear Notifications </a>
        </Menu>
      ...
      >
        <Col span={16} className="logoDiv">
          ...
          <div className="innerlogo">
            ...
            <Link to="/">
              <img src={logo}/>              
              <span className="mobile_invisiable " style={{ fontSize: "24px", fontWeight: "600", marginLeft: "-10px", }}> Galinukkad </span>
             </Link>
              
            </h3>
          </div>
        </Col>
        <Col span={8} className="topmenu">
