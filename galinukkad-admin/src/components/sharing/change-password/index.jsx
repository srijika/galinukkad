...
const ChangePassword = props => {
  ...
  const  handelPassword2 = () => {
		setShowPassword2(!showPassword2);
		
	}
  const  handelPassword3 = () => {
    ...
    <Form layout="vertical" form={form} name="resetpass" className="login-form" onFinish={onFinish} >
      ...
      <Tooltip placement="right" title={content} trigger={'focus'} overlayClassName="intooltip" visible={TooltipShow}>
        <Form.Item name="password" label="NEW PASSWORD"
          ...
          { max: 16, message: 'Password must not be greater than 16 length.' },
          {
        ...
        >
          <Input  className={(valid ? 'hasEyeIcon' : '') + ' hasLeftPrefix'} prefix={<i className="fas fa-key" name="key" />} placeholder="Password"
            //autoFocus={true}
            //ref={fieldFocus}
            maxLength="16"
            suffix={suffixpass}
            type={showPassword2 ? 'text': 'password'}
            suffix = {showPassword2 ? <EyeTwoTone onClick={ handelPassword2 } /> : <EyeInvisibleOutlined onClick={ handelPassword2 } /> }
      ...
      >
        <Input  className="hasLeftPrefix hasEyeIcon"
          prefix={<i className="fas fa-key" name="key" />}
          maxLength="16"
          type={showPassword3 ? 'text': 'password'}
          suffix = {showPassword3 ? <EyeTwoTone onClick={ handelPassword3 } /> : <EyeInvisibleOutlined onClick={ handelPassword3 } /> }
