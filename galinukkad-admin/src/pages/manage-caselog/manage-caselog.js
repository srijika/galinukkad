...
class ManageCaseLog extends React.Component {
  ...
  componentDidMount() {
		setTimeout(() => document.title = 'User Help', 100,);
  ...
  render() {
    ...
    const productsAvilabilityColumns = [
      ...
      },
      {
        ...
        width: 150,
        render: (val, data) => {
          return data.seller ? data.seller.username.substr(0, 20) : ''
        }
      },
      {
        title: 'Mobile/Email',
      ...
      },
      {
        ...
        render: (val, data) => {
          return <div>
            {/* <Button style={{ textAlign: 'center', width: '100%' }} onClick={() => this.openDetailsModal(data)} type="primary"  >Query</Button> */}
            <Button onClick={() => { 
   
               this.props.history.push(`/user/${data.loginid}/${data._id}/queries`) }}
               style={{ marginBottom: '5px', textAlign: 'center', width: '80px', marginRight: '5px' }} type="default"><span style={{ fontSize: "12px" }}>Query</span></Button>
            <Button style={{ marginBottom: '5px', textAlign: 'center', width: '80px', marginRight: '5px' }} onClick={() => this.openDetailsModal(data)} type="primary"  ><span style={{ fontSize: "12px" }}>Details</span></Button>
           ...
           {data.status === 1 ?  <Button style={{ marginTop : '5px',textAlign: 'center', width: '80px' }} onClick={() => this.deleteCaseLog(data)} type="danger"><span style={{ fontSize: "12px" }}>Close</span></Button>
          : ''}
         </div>
        }
      },
