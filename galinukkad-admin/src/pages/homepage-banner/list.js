...
class HomepageBanner extends React.Component {
  constructor(props) {
    ...
    this.update = false;
		this.state = {
			limit: 25, current: 1, sortBy: 'asc', showModal: false, detail:null,banners:[], searchText: '', listData: [] , A : true
		}
    ...
    render(){
        ...
        const columns = [
          ...
          },
          {
            ...
            width:100,
            render:(val,data) => {
              return val === "" || val === "null" || val === "undefined" ? "No url" : <span className="text-info">{val}</span>
            }
          },
          {
            title: 'Banner Size',
          ...
          },
          {
            ...
            return (
              <div onClick={e=>e.stopPropagation()}>
                <Button type="primary" className="ant-btn-sm mr-2" onClick={(event) => {

                this.setState({showModal:true, detail:r, isAdd:false })
                }
                } ><EyeOutlined /></Button>&nbsp;
                <Popconfirm title="Are you sure delete this Banner?" onConfirm={e=> {this.deleteBanner(r._id); e.stopPropagation()}} okText="Yes" cancelText="No" >
