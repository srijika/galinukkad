...
import { UserOutlined, EditOutlined, EyeOutlined, DeleteOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
	...
	render() {
		...
		const columns = [
			...
			{ title: <strong>Action</strong>, width: 140, render: (val, data) => 
				roleType == 'ADMIN' ? 
				<div>
					<Button  className="ant-btn-sm" type="primary" style={{marginBottom : '4px' , marginRight : '4px'}} onClick={()=>{this.props.history.push('/news/edit/' + data._id )}}><EyeOutlined /></Button>

					{/* <Button className="ant-btn-sm" type="primary" onClick={e => { this.viewItem(data); e.stopPropagation() }}><EyeOutlined /></Button> */}
					{/* <Button className="ant-btn-sm" type="danger" onClick={e => { this.deleteItemFun(data._id); e.stopPropagation() }}><DeleteOutlined /></Button>  */}
					<Popconfirm title="Are you sure delete this news?" onConfirm={e => { this.deleteItemFun(data._id); e.stopPropagation() }} okText="Yes" cancelText="No" >
						<Button  className="ant-btn-sm" type="danger" ><DeleteOutlined /></Button>
					</Popconfirm>
				</div>
				: <Button className="ant-btn-sm" type="primary" onClick={e => { this.viewItem(data); e.stopPropagation() }}><EyeOutlined /></Button>
			},
