...
import { Card, Typography, Input, Button, Table, Row, Col, Avatar, Tabs, Modal, message , Popconfirm } from 'antd';
...
export default function AnnouncementList(props) {
	...
	useEffect(() => {
		...
		getAnnouncements();
		setTimeout(() => document.title = 'Announcement', 100,);
	...
	const columns = [
		...
		{ title: <strong>Action</strong>, width: 100, render: (val, data) => 
				...
				<Popconfirm title="Are you sure delete this Announcement?" onConfirm={() => { handleDelete(data._id) } } okText="Yes" cancelText="No" >
