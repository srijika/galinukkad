...
import {Col, Modal, Card,message,  Alert,Form,Input, Checkbox,Button,Switch, Upload, Dropdown, Menu, Select, notification, Transfer, DatePicker } from 'antd';
...
const { Option } = Select;
export default function MultiSupport(props) {
  ...
  useLayoutEffect(() => {
    getTicketQueries();
    getTickets();
setTicketMessageQueries((oldArr) => [...oldArr, messages]);
...
if(role === "ADMIN"){
  ...
  data = {
    ...
    const res = await axios.post(`${baseUrl}/send/message-query`, data);
...
return false;
      ...
      const  data = {
          ...
          const response = await axios.post(`${baseUrl}/send/message-query`, data);
...
setTimeout(() => {
  ...
  const getTickets = async () => {
    const res = await axios.post(`${baseUrl}/get/tickets/index`, { user_id: id });
  ...
  const getTicketQueries = async () => {
    ...
    const res = await axios.post(`${baseUrl}/ticket-queries/index`, data);
 ...
 // const getTicketNotificationReadUrl = async (_id) => {
  ...
  const onFinishForm= async val=>{
    ...
    const res = await axios.post(`${baseUrl}/admin/create/ticket`, data);
    const  datas = {
      from: id,
      to: "ADMIN",
      message: val.message,
...
message.success("Ticket Created!", 5)
props.history.push(`/seller/${id}/${res.data.result._id}/queries`);
setMessage(datas)
                  ...
                  return (
                    ...
                    >
                      ...
                      ''
                        ...
                        <hr />
                        {item.status === 1 ? role === "ADMIN" ? <span>  <button type="button" class="btn btn-danger btn-xs " onClick={() => { handleDelete(`${baseUrl}/ticket/${item._id}/close` , item._id , item.user_id ,   ) } }>Close Ticket</button></span> : ''
...
{status && status === 1 ? <Form {...formItemLayout} form={form} name="ticket_rply" layout="vertical" onFinish={onFinish} className="innerFields">
  <Col sm={22} md={22}>
						<Form.Item name="message" label="Reply" rules={[{ required: true, message: 'Please enter some value!' },{ max: 100, message: 'Reply must be less than 100 characters.' },]} >
                        	<TextArea placeholder="Type your reply here..." type="number" rows={4}/>
						</Form.Item>
					</Col>

          <Form.Item className="mb-0">
				<Button type="primary" disabled={btnDis} className="btn-w25 btn-primary-light mb-4" onClick={() => form.submit()}>Send</Button>
				</Form.Item>
  ...
  '' }
      ...
      <Modal visible={visiable} title="Create Ticket" onCancel={cancelFun} footer={<Fragment>
			...
			{role === "ADMIN" ?	<Button type="primary" disabled={btnDis} className="btn-w25 btn-primary-light" onClick={()=>form.submit()}>Create Ticket</Button> : ''}
			</Fragment>} >
		<Form  form={form} name="loc_info" layout="vertical" onFinish={onFinishForm} className="innerFields">
			<Form.Item name="title" label="Title" rules={[{ required: true, message: 'This field is required!' },{ max: 50, message: 'Title must be less than 50 characters.' }]} >
      ...
      <Form.Item name="message" label="Message" rules={[{ required: true, message: 'This field is required!' },{ max: 100, message: 'Message must be less than 100 characters.' }]} >
