...
import { SketchOutlined , SendOutlined} from '@ant-design/icons'; 
const dateFormat = 'DD/DD/YYYY';
...
const AddEdit =props => {
	...
	useEffect(() => {
		...
		console.log('data.expiryDate' , data.expireDate)
		if(props.detail){form.setFieldsValue({
			  ['name']: data.codeTitle, 
			  ['code']: data.code, 
			  ['expireDate']: data.expireDate && moment(new Date(data.expireDate) || null, dateFormat)	,
...
return (
	...
	{<Fragment>
		...
		<Form {...formItemLayout} form={form} name="loc_info" layout="vertical" onFinish={onFinish} className="innerFields">
			...
			<Form.Item  name="code" rules={[{ required: true, message: 'This field is required!' },{ max: 20, message: 'Code must not be greater than 20 characters.' }]} >
				<Input placeholder="Coupon Code" />
			</Form.Item>
				<button type="button" className="btn btn-success" style={{ height: '36px', width: '46px' }}
            ...
            pattern: /^[\d]{0,7}$/,
            message: "MRP Price should be less than 7 character",
          },]} >
				<Input type="number" placeholder="Amount" />
			</Form.Item>

			<Form.Item name="codeDescription" rules={[{ required: false, message: 'This field is required!' },{ max: 200, message: 'Description must not be greater than 200 characters.' }]} className="mb-0">
