...
class UsersList extends React.Component {
  constructor(props) {
	...
	ListFun=()=>{
		...
		let searchval = {limit:this.state.limit, role:"CUSTOMER"}
	...
	const columns = [
	  ...
	  { title:<strong> Status </strong>, dataIndex: 'deactive', render:(val,data)=> 
	 		<div>
				...
				<Popconfirm title={`Are you sure you want to ${data.deactive ? "activate" : "deactivate"} this user?`} onConfirm={e=> {this.handleDeactiveUser(data._id)}} okText="Yes" cancelText="No" >
					<Button type="primary mobile_acdc" > {data.deactive ? "Activate" : "Deactivate"}  </Button>
				</Popconfirm>
	  ...
	  { title:<strong>Action</strong>, width:200, //align:'center',
		...
		<div>
			 <Button  title="Ticket Query" onClick={() => { 
                ...
                this.props.history.push(`/user/${data._id}/queries`) }}
               className="mr-2 mb-2 mobile-btn"><CommentOutlined /></Button>
			<Button type="primary" title="View User Details" onClick={()=>this.props.history.push('/users/edit/'+data._id)}><EyeOutlined /></Button>&nbsp;
		<Popconfirm title="Are you sure delete this user?" onConfirm={e=> {this.deleteItem(data._id)}} okText="Yes" cancelText="No" >
		<Button title="Are you sure delete this user" type="danger" ><DeleteOutlined /></Button>
	  </Popconfirm>
	  </div>
	  },
	];
