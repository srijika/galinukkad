			...
			if(!unmounted &&  detail && detail.status){
			  ...
			  let data = detail.userLogin;
				setDetail({
					...data,
					dob: data.dob || null,
					gstin: data.fssai && data.fssai == true ? 'Yes' : 'No',
					fssai: data.fssai && data.fssai == true ? 'Yes' : 'No',
...
return (
	<Card title={<span><LeftOutlined onClick={()=> props.history.push('/seller')}/> Seller Info</span>} style={{marginTop:"0"}}>
	
		 <Descriptions size={'middle'} bordered>
          <Descriptions.Item label="Seller Name">{detail && detail.username || ''}</Descriptions.Item>
		  ...
		  <Descriptions.Item label="FSSAI">{detail && detail.fssai  || ''}</Descriptions.Item>
          <Descriptions.Item label="GSTIN">{detail && detail.gstin || ''}</Descriptions.Item>
        </Descriptions>
