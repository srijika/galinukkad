...
class PagesList extends React.Component {
	...
	componentDidMount() {
		this.ListFun();
		this.props.dispatch({ type: 'blogs/clear' });
	...
	searchVal = (val) => {
		...
		const resultAutos = this.props.blogs.list.filter((auto) => auto.title.toLowerCase().includes(val.toLowerCase()))
		console.log('val' , val)
		console.log('this.props.blogs' , this.props.blogs)
		console.log('resultAutos' , resultAutos)
		...
		const columns = [
			...
			},
			{
				...
				render: (val, data) => <div onClick={e => e.stopPropagation()}>
					<Button type="primary" className="ant-btn-sm mr-2" onClick={()=>{this.props.history.push('/blogs/edit/' + data._id )}}><EyeOutlined /></Button>
					<Popconfirm title="Are you sure delete this blog?" onConfirm={e => { this.deletePages(data.slug); e.stopPropagation() }} okText="Yes" cancelText="No" >
