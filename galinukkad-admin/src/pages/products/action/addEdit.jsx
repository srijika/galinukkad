...
if(date === "" || date === null){
	...
	const convertToFormData = (val) => {
		...
		const formData = new FormData();
			formData.append("is_manage_stock",manageStock)
...
// notification.error({message: `${item.label} Variant value are required`});
message.error( `${item.label} Variant value are required`, 5);
		...
		console.log("manageStock" , manageStock)
	...
	const onFinish = val => {
		...
		if(val.price > val.mrp_price){


message.error( "Price must be lesser than MRP Price", 5);
...
// notification.error({message: "Gallery images field required"});
message.error( "Gallery images field required", 5);
...
// notification.error({message: "Please Enter Variants and Values"});
message.error( "Please Enter Variants and Values", 5);
		...
		if (props.match.params.id) {
			...
			if (!unmounted && detail && detail.status) {
				...
				form.setFieldsValue({
					...
					// ['is_manage_stock']: data.is_manage_stock,
				...
				setManageStock(data.is_manage_stock);
	...
	const uploadButton = (
		<div>
			{loading ? <LoadingOutlined /> : <i className="fad fa-camera-retrow" style={{ color: "#13c2c2", fontSize: 57 }} />}
		</div>
	);
	...
	//   }
	  ...
	  const onUploadGallery = (data) => {
		  console.log('onUploadGallery' , data)
	  ...
	  const onUploadGallerychange = (data) => {
		console.log('onUploadGallery' , data)
					...
					<Col flex="auto">
						<Row gutter={15}>
							<Col  xs={24} sm={24} md={12}>
								<Form.Item name="title" label="Product Name" rules={[{ required: true, message: 'Field required!' },{ max: 50, message: 'Product Name must not be greater than 50 characters.' },]}  >
						...
						<Row gutter={15}>
							...
							<Col xs={24} sm={24} md={12}>
								<Form.Item name="mrp_price" label="MRP Price"  rules={[{  required: true, message: 'Numeric Value Required' }, {
            pattern: /^[\d]{0,7}$/,
            message: "MRP Price should be less than 7 digit",
          }, ]}>
									<Input  max={7} placeholder="MRP Price" type="number" onChange={(e) => {setMrpPrice(e)}} />
								</Form.Item>
							</Col>
							...
							<Col xs={24} sm={24} md={12}>
								<Form.Item name="price" label="Price" rules={[{  required: true, message: 'Numeric Value Required' },{
            pattern: /^[\d]{0,7}$/,
            message: "Price should be less than 7 digit",
          }
							]} >
									<Input min={0}  max={7} placeholder="Price" type="number" onChange={(e) => { setPrice(e)}} />
								</Form.Item>
{price > mrpPrice && <div className="text_danger">Price must be lesser than MRP Price</div>}
						...
						<Row gutter={15}>
							<Col  xs={24} sm={24} md={12}>
								...
								<Form.Item name="inventory" label="Stock Quantity" rules={[{ required: true, message: 'Numeric Value Required' },{
            pattern: /^[\d]{0,7}$/,
            message: "Stock Quantity should be less than 7 Digit",
          }]} >
						...
						<Row gutter={15}>
							...
							<Col  xs={24} sm={24} md={12}>
								<Form.Item name="brand" label="Brand Name"  rules={[{ required:false },{ max: 50, message: 'Brand Name must not be greater than 50 characters.' },]}   >
				...
				<Row gutter={15}>
					<Col  xs={12} sm={12} md={12}>
						<Form.Item name="sku" label="Sku" rules={[{ required: true, message: 'This field is required!' },{ max: 50, message: 'Sku must not be greater than 50 characters.' },]}  >
								<Input placeholder="Sku" />
						</Form.Item>
					</Col>
					
					<Col  xs={12} sm={12} md={12}>
						<Form.Item label="Manage Stock?" name="is_manage_stock" >
							<Checkbox checked={manageStock} onChange={onManageStockChange} ></Checkbox>
						</Form.Item>
					</Col>
					...
					<Col  xs={24} sm={24} md={24}>
						<Form.Item name="low_stock_threshold" rules={[{
            pattern: /^[\d]{0,4}$/,
            message: "low stock should be less than 4 digit",
          }, ]} >
				...
				<Col  xs={24} sm={24} md={24} style={{ position: "relative" }}>
					<Form.Item name="weight" label={<><span> Product Weight </span> 	<b style={{fontSize: "10px" ,'color':'rgb(225 106 106)'}}> &nbsp;( Please type product weight in grams. )</b></>} 
						rules={[{ required: true, message: 'This field is Required' } ,{
            pattern: /^[\d]{0,10}$/,
            message: "weight should be less than 10 digit",
          }, ]} 
					>
						<Input placeholder="Product Weight(gms)" type="number"	 />
					</Form.Item>
					
					{/* <span style={{  color: "rgb(225 106 106)", fontSize: "10px" }}> * Please type product weight on grams </span> */}
				</Col>
...
{errorDesLength > 1000 && 'you cross the limit of description'}
								...
								<Row gutter={15} key={index8} >
									<Col sm={24} md={11}>
										...
										>
											...
											<Input className="form-control2" type="text" disabled />
									...
									<Col sm={4} md={2}>
										<Form.Item className="remove_icn">
											<MinusCircleOutlined className='text-danger' onClick={() => remove(field.name)} />
										</Form.Item>
									</Col>
									</Row>
									</Col>
				...
				<Row gutter={15}> 
					<Col  xs={24} sm={24} md={24}>
						...
						label={<><span> Thumbnail Images </span> 	<b style={{fontSize: "10px" ,'color':'rgb(225 106 106)'}}> &nbsp;( Please Use 170px * 170px Image to upload. )</b></>} 
							// <span style={{ color: 'red' }}>*</span>

						>
							{imagesList.length >0 && imagesList.slice(0, 1).map((item, index) => 
								item.urls ? <Avatar key={index} shape="square"  src={item.urls } className="avatart_img" style={{margin:5 , width: "auto" ,height : "150px"  }} /> :
								<Avatar key={index} shape="square"  src={process.env.REACT_APP_ApiUrl+'/'+ item.url} style={{ margin: 5 , width: "auto" , height : "150px" }} />
							)}
				...
				<Row gutter={15}>
					<Col  xs={24} sm={24} md={24}>
						...
						label={<span> <span className="rqr_field">*</span>Gallery Images </span>} >
							 <MultiImageInput
								...
								theme="light"
								allowCrop = {false} 
								onChange={onUploadGallerychange}
					...
					<Col className="mb-4"  xs={24} sm={24} md={4}>
				...
				<Button type="primary"  disabled={btnDis} className="btn-w25 btn-primary-light save_btn" onClick={() => form.submit()}>{props.product.detail ? 'Update' : <span className='add_btn' style={{paddingLeft: 10 , paddingRight: 10}}>Add</span>}</Button>
				</Form.Item>
			</Form>
				{
			visible && 
			<UploadImages 
			 ...
			 visible={visible} closeFun={() => setVisible(false)} 
			returnImg={
