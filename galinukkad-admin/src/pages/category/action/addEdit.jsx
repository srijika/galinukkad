...
return (
	...
	<Modal visible={props.visible} title={props.detail?'Edit Category':'Add Category'} onCancel={cancelFun} footer={<Fragment>
		...
		<Form {...formItemLayout} form={form} name="loc_info" layout="vertical" onFinish={onFinish} className="innerFields">
			...
			{ min: 3, message: 'Slug must be minimum 3 characters.' },
			{ max: 25, message: 'Slug must be less than 25 characters.' },
		]} >
			...
			<Form.Item name="description" label="Description" rules={[{ required: true, message: 'This field is required!' },{ max: 100, message: 'Description must not be greater than 100 characters.' }]} >
            ...
            pattern: /^[\d]{0,2}$/,
            message: "GST should not be greater than 99%",
          }]} >
			{/* <Form.Item name="gst" label="GST %"  rules={[{ required: false },{max : 2 , message: 'GST should not be greater than 99%'}]} > */}

				<Input  max={7} placeholder="GST"  type="number" />
			</Form.Item>	
			...
			<div>
				<input type="file" name="image"  onChange={(e) => { imageFun(e) }} ref={fileInputRef}  />
