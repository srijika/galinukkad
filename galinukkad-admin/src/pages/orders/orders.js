  ...
  constructor(props) {
    ...
    render(){
        ...
        const columns = [
          ...
          {
            title: <strong className="primary-text cursor">Product<i className={' fal fa-sm '+(this.state.sortBy === 'asc'? "fa-long-arrow-up":"fa-long-arrow-down")} ></i></strong>,
            dataIndex: 'orderNo',
            render:(val,data) => {
              console.log("data" , data)
              return data && data.products.map((val, i)=> {
                console.log("val____", val.product.title)
return(
<>
<span className="product_title">({i + 1})  {val.product.title.substr(0, 50)}</span><br />
</>)
              })
              ...
              return (
                <div> 
                    <div style={{display:"flex", alignItems:'center', justifyContent:'flex-start'}}>
                        <Button style={{  textAlign:'center', width:'100%' , }}  onClick={()=> this.props.history.push('/order/'+data._id)} type="primary"><EyeOutlined /></Button>
                  </div>
                </div>
              );
             } 
