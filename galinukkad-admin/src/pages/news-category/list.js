...
paginationFun = (val) => this.setState({ current: val.current }, () => this.ListFun());
  ...
  render() {
    ...
    const productsAvilabilityColumns = [
      ...
      // {
      //   title: <strong>Updated Date</strong>,
      //   dataIndex: 'updated_at',  
      //   render:(val,data) => {
      //     return (data.created_at? <Moment format="MM- DD-YYYY" >{data.updated_at}</Moment>:'-')
      //   }
      // },
      {
        ...
        render: (val, data) => <div onClick={e => e.stopPropagation()}>
          ...
          <Popconfirm title="Are you sure delete this News Category?" onConfirm={e => { this.deleteCat(data._id); e.stopPropagation() }} okText="Yes" cancelText="No" >
