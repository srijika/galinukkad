...
class NewsCategory extends React.Component {
  constructor(props) { 
    ...
    setTimeout(() => document.title = 'Blogs Category List', 100);
  }
  componentDidMount() {
    this.ListFun();
    this.props.dispatch({ type: 'blogsCategory/clearAction' });
  }
...
if (user.role === "ADMIN") {
  ...
  this.props.dispatch({ type: 'blogsCategory/blogsCategoryList', payload: searchval, });
} else {
  let search = 'page=' + (this.state.current - 1) + "&limit=" + this.state.limit + "&inactive=" + this.state.inactive + "&searchText=" + this.state.searchText + "&sortBy=" + this.state.sortBy;
  localStorage.setItem('newsSearch', JSON.stringify(this.state))
...
paginationFun = (val) => this.setState({ current: val.current }, () => this.ListFun());
  ...
  getSnapshotBeforeUpdate(prevProps, prevState) {
		...
		if (this.props.blogsCategory.del && this.props.blogsCategory.del.count > this.state.count && this.props.blogsCategory.del.status) {
			this.setState({ count: this.props.blogsCategory.del.count })
			return true
		}
		if (this.props.blogsCategory.add && this.props.blogsCategory.add.count > this.state.Addcount && this.props.blogsCategory.add.status) {
			this.setState({ Addcount: this.props.blogsCategory.add.count, addModel: false })
			return true
		}
		return null;
	}
  ...
  render() {
    ...
    const { blogsCategory } = this.props;
    console.log('blogsCategory', this.state.listData);
    const total = 0; 
    const totalActive = 0;
    if (this.state.searchText == '') {
      this.state.listData = blogsCategory.list ? blogsCategory.list.result : [];
    }
    const productsAvilabilityColumns = [
      ...
      },
      {
        ...
        render: (val, data) => <div onClick={e => e.stopPropagation()}>
            {/* <Button type="primary" style={{marginRight: '5px' ,}} onClick={() => this.setState({ addModel: true, detail: val }) }  > <EyeOutlined /> </Button> */}
					<Button type="primary" className="ant-btn-sm mr-2" onClick={() => this.setState({ addModel: true, detail: val }) }><EyeOutlined /></Button>

          <Popconfirm title="Are you sure delete this Blog Category?" onConfirm={e => { this.deleteCat(data._id); e.stopPropagation() }} okText="Yes" cancelText="No" >
...
export default connect(({ blogsCategory, loading }) => ({
  blogsCategory, 
  loading
}))(NewsCategory);
