...
const  handelPassword = () => {
	...
	useEffect(() => {
		...
		if(!unmounted && register.action && register.action.status === true && register.action.count > count){
			...
			setTimeout(() =>  {
				Modal.success({
					content:( 'Registered Successfully! Galinukkad team will contact you soon'),
					  onOk() { 
				...
				<p class="login-box-msg">Register Now</p>
					...
					<Form ref={formRef} layout={'vertical'} name="normal_login" className="login-form" initialValues={{ remember: true, }} onFinish={onFinish} >
					  ...
					  <Form.Item name="password" rules={[ 
							...
							{ max: 20, message: 'Password must not be greater than 20 length.' },
						]} >
					  ...
					  <Form.Item name="gstin" label="Do you have GSTIN?" rules={[ { required: true, message: 'Please select GSTIN!', }, ]} >
						<Radio.Group >
					  ...
					  <Form.Item name="fssai" label="Do you have FSSAI?"
					   >
						<Radio.Group > 
							<Radio value={true}>Yes</Radio>
							<Radio value={false}>No</Radio>
					  ...
					  <Form.Item>
						<Button type="primary" htmlType="submit" className="login-form-button mobile-register-form-button"> Register Seller </Button> <br /><br />
