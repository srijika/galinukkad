...
const AppLogin = (props) => {
	...
	useEffect(() => {
		let unmounted = false;
			...
			if(flow && flow == 'sin' && varify.status){
				...
				//props.dispatch({ type: 'auth/login', payload: JSON.parse(LoginCred)});
			}
		return () => {
			unmounted = true;
		}
	...
	useEffect(() => {
		...
		let check = localStorage.getItem('forgotpassword');


		if(!unmounted && forgot.count > fcount && forgot.status && check != 'false'){
	...
	<div>
		...
		<Row type="flex" className="basicpage mobile_login" justify="space-around" align="middle">
			...
			<Col  className="basicbox">
				...
				<img className=" elevation-3 img-fluid mobile_login_logo" src="https://galinukkad.com/static/img/index.png" />
				<p class="login-box-msg">Login to start your session</p>
					...
					<Form name="normal_login" className="login-form" initialValues={{ remember: true, }} onFinish={onFinish} >
					  <Form.Item name="username" rules={[ { required: true, message: 'Please enter your email or phone number!', }, ]} >
						<Input className="mobile-form-control" prefix={<UserOutlined className="site-form-item-icon" />} placeholder="Email or phone number" />
					  </Form.Item>
					  ...
					  <Form.Item> 
						<a className="login-form-forgot mobile-login-form-forgot" onClick={()=>setVisible(true)} >
						  Forgot password
						</a>
					  </Form.Item>

					  <Form.Item>
						<Button type="primary" htmlType="submit" className="login-form-button mobile-login-form-button"> Log in </Button> <br /> <br />
