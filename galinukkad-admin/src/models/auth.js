import { login, resetpass, Register, forgetpassword, resetPassword, resendOTPTOUser, varifyUser } from '../services/api'
  ...
  effects: {
    ...
    *login({ payload }, { call, put }) {
      ...
      if (!response.status) {
        ...
        message.error("Please Verify Your Account", 5);
      ...
      if (response.status) {
        ...
        let activeRole = '';
        if (hostname == 'seller.galinukkad.com') {
          activeRole = 'SELLER'
        } else if (hostname == 'admin.galinukkad.com') {
          activeRole = 'ADMIN'
        } else {
          activeRole = 'SELLER'
        }
        ...
        // if (role === activeRole) // For Production

        if(window.location.hostname === 'localhost') {
            let role = response.user.role;
            if (role !== "CUSTOMER" && role !== "GUEST") //For localhost 
            {
    ...
    *register({ payload }, { call, put }) {
      ...
      if (!response.status) {
        if (response.msg == "This Email is already registered" || response.message == "This Email is already registered") {
          message.success("Your email is already register in galinukkad so login with your email & password..!", 5);
          ...
          message.success("Your mobile number is already register in galinukkad so login with your mobile number & password..!", 5);
          yield put({ type: 'alrdreg', ...response });
        } else {
          message.error(response.msg || response.message || response.err, 5);
        }
