import { getAdvPlanList, getNewsList, createNews, updateNews, newsDetail,deleteNews } 
from '../services/api'
...
export default {
  ...
  effects: {
    ...
    *newsList({ payload }, { call, put }) {
      ...
      *newsDetail({ payload }, { call, put }) {

        const response = yield call(newsDetail, payload._id);
        if (!response.status) { message.error(response.msg || response.message || response.err, 5); }
  ...
  reducers: {
    ...
    clear(state, action) {
      return { ...state, detail:{}, add:{count:0}, edit:{count:0}, del:{count:0}};
    },
  },
};