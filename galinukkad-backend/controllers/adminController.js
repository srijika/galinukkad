...
exports.dashBoard = async (req, res, next) => {
    ...
    try {
        ...
        let Total_Products ;
        const getUserRole = await UserLogins.findOne({_id : loginid})
        let roleType = getUserRole.roles;

        let countOrder = await Order.find();
        let sellerOrder
        let cancel_Orders
        let cancleorderItems 
        let returnItems 
        let SameDayItemReturn

        const sameDate = moment().startOf('day')
        //console.log(orderItems) ;

        if(roleType === "ADMIN"){
           
            Total_Products  = await Product.countDocuments(isSeller ? { loginid: loginid } : {});
            sellerOrder     = await OrderDetail.find({}).count().lean().exec();  
           
            //cancel_Orders   = await Order.find({status : '2'}).count();
            cancel_Orders   = await OrderDetail.find({item_status : '2'}).count();
            returnItems     = await OrderDetail.find({item_status : '3'}).count();
            SameDayItemReturn  = await OrderDetail.find({ $and: [ { item_status : '3' },{
                create: {
                  $gte: sameDate.toDate(),
                  $lte: moment(sameDate).endOf('day').toDate()
                }
              }  ] }).count();
            ...
            sellerOrder   = await OrderDetail.find({ seller_id:loginid }).count();
           
           
            cancleorderItems   = await OrderDetail.find({ $and: [ { item_status : '2' }, { seller_id:loginid }  ] }).count();
            returnItems        = await OrderDetail.find({ $and: [ { item_status : '3' }, { seller_id:loginid }  ] }).count();
            SameDayItemReturn  = await OrderDetail.find({ $and: [ { item_status : '3' }, { seller_id:loginid },{
                create: {
                  $gte: sameDate.toDate(),
                  $lte: moment(sameDate).endOf('day').toDate()
                }
            ...
            let Total_Products1 = await Product.find({loginid : loginid});
          
            const countOrders = await Order.find();
            // const cancelItems = await OrderDetail.find() ; 
            // console.log(cancelItems);
            const filterorders = countOrders.map((val , i) => {
            ...
            if(filterorders.length > 0) {
                ...
                // const flat =filterorders.flat()
                    let seller = filterorders.filter((val , i) => {
                        return val === loginid
                })
                
                const filterorderss = countOrders.map((val , i) => {
                    return {"a" :val.seller_id , "b" : val.status}
                })
               
                
                let seller_cancel = filterorderss.filter((val , i) => {
                    return val.a[0] == loginid && val.b === 2
                })
                // console.log(seller_cancel.length)
                
               // sellerOrder = (filterorders.length > 0 ) ? filterorders.length  : 0 ;
              
               
                //cancel_Orders = seller_cancel.length
                cancel_Orders = cancleorderItems ;
            }


            Total_Products = Total_Products1.length;
        ...
        const toDaySale = resultOrderData.length ;
        const toDayReturn = SameDayItemReturn;
        ...
        const DBdata = {
            ...
            get_Total_Orders: sellerOrder,
            get_Cancel_Orders: cancel_Orders,
            //returnItems
            //get_Total_Refund_Orders: total_Refund_Orders.length,
            get_Total_Refund_Orders: returnItems,
