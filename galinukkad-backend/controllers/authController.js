...
module.exports = {
    ...
    loginNew: async (req, res, next) => {
        try {
            ...
            // db.collection.find({'email': {'$regex': Username,$options:'i'}});
           
            let isUser = await UserLogins.findOne({ $or: [{ 'email': new RegExp(`^${Username}$`, 'i') }, { mobile_number: Username }] })
            ...
            if (comparePass || (!comparePass && isUser.password === Password)) {
                ...
                if (isUser.roles !== ROLES[3]) {
                    if (!isUser.isMobileVerified && !isUser.isEmailVerified) {
                        ...
                        Helper.sendEmail(isUser.email, "OTP Verification", msg_body);
                        await UserLogins.findByIdAndUpdate(isUser._id, { otp_time:  new Date()   });

                        return res.send({ status: false, email : isUser.email,message: "Your account is not verified" });
                    }
                }
                ...
                let userData = { username: isUser.username, email: isUser.email, mobile: isUser.mobile_number, isMobileVerified: isUser.isMobileVerified, isEmailVerified: isUser.isEmailVerified, _id: isUser._id, time: new Date().getTime(), role: isUser.roles, 
                    isBussinessVerified: isUser.isBussinessVerified
                };
    ...
    sendOtpToUser: async (req, res, next) => {
        try {
            ...
            return res.send({ status: false, err: error.message });
        }
    },
...
let today = new Date();
let Otp_day = isUser.otp_time;
let diffMs = (today - Otp_day); // milliseconds between now & Otp_day 
let diffDays = Math.floor(diffMs / 86400000); // days
let diffHrs = Math.floor((diffMs % 86400000) / 3600000); // hours
let diffMins = Math.round(((diffMs % 86400000) % 3600000) / 60000); // minutes
console.log( diffMins + " minutes until Otp_day =)_______________");
console.log(diffMins >= 5);
console.log(diffMins >= 1);


if(diffMins >= 1){
    const userOtp = generateOTP();
    // await UserLogins.findByIdAndUpdate(isUser._id, { otp_time:  new Date()  ,otp:userOtp });
    return res.send({ status: false, message: "OTP is Expires" });
...
if (isUser.otp == otp || isUser.mobile_otp == otp) {
    ...
    signup: async (req, res, next) => {
        ...
        // try {
            ...
            const isUser = await UserLogins
                .findOne({ $or: [{ 'email': new RegExp(`^${email}$`, 'i') }, { username: username }, { mobile_number: mobile_number }] })
                .lean().exec();
            ...
            return res.send({ status: true, message: `${roleType} Sign Up Successfully`});

            // return res.send({ status: true, data: userLoginCreate, profile: profileCreate , message: `${roleType} Sign Up Successfully`});
    ...
    userActiveDeactiveStatus: async (req, res, next) => {
        try {
            ...
            UserLogins.findById(id, function(err, data) {
                ...
                console.log("data" ,data.deactive)
                let status = data.deactive === false ? "Activated" : "Deactivated";

                

                let msg_body = `Hi, ${data.username} <br />`;
                msg_body += `Your Account ${status} By Admin ` ;
                Helper.sendEmail(data.email, "Account Status", msg_body);
    ...
    maintenanceMode: async (req, res, next) => {
      ...
      // User Import export from Admin --------------------------------

      getAllUsersExport: (req, res, next) => {
        ...
        try {
            ...
            UserLogins.aggregate([
                ...
                for (let i = 0; i < data.length; i++) {
                  if(data[i].profile){
                    data[i].profile_create = data[i].profile.create;
                  }
                  if(data[i].business_profile){
                    data[i].store_name = data[i].business_profile.storeName;
                    data[i].panNumber = data[i].business_profile.panNumber;
                    data[i].bemail = data[i].business_profile.bemail;
                    data[i].address = data[i].business_profile.address;
                  }
...
const reportInfo = {
    reportName: 'Sellers',
    arrayData: data,
    fileName: `Sellers-${Date.now()}.xlsx`,
    columns: [
                    { header: 'User Name', key: 'username', width: 15 },
                    { header: 'Email', key: 'email', width: 20 },
                    { header: 'Is Email Verified', key: 'isEmailVerified', width: 20 },
                    { header: 'Phone', key: 'mobile_number', width: 20 },
                    // { header: 'Is Mobile Verified', key: 'isMobileVerified', width: 20 },
                    // { header: 'Role', key: 'roles', width: 20 },
                    // { header: 'Profile Created On', key: 'profile_create', width: 20 },
                    { header: 'FSSAI', key: 'fssai', width: 20 },
                    { header: 'GSTIN', key: 'gstin', width: 20 },
                    // { header: 'Store Name', key: 'store_name', width: 20 },
                    // { header: 'PAN Number', key: 'panNumber', width: 20 },
                    // { header: 'Business Email', key: 'bemail', width: 20 },
                    // { header: 'Business Address', key: 'address', width: 50 },
...
const rows2 =[
    { field_name: 'User Name', details: 'An alphanumeric string up to 200 characters.', example: 'username:James' },
...
if (Report.status) {
    ...
    importUsers: async (req, res, next) => {
        try {
           ...
           const readExcel =  await readXlsxFile(req.files[0].path).then( async(rows) => {
                const columns = [
                    { title: 'User Name', slug: 'username'},
                    { title: 'Email', slug: 'email' },
                    { title: 'Is Email Verified', slug: 'isEmailVerified' },
                    { title: 'Phone', slug: 'mobile_number' },
                    // { title: 'Is Mobile Verified', slug: 'isMobileVerified' },
                    // { title: 'Role', slug: 'roles' },
                    // { title: 'Profile Created On', slug: 'profile_create' },
                    { title: 'FSSAI', slug: 'fssai' },
                    { title: 'GSTIN', slug: 'gstin' },
                    // { title: 'Store Name', slug: 'store_name' },
                    // { title: 'PAN Number', slug: 'panNumber' },
                    // { title: 'Business Email', slug: 'bemail' },
                    // { title: 'Business Address', slug: 'address' },
                ...
                for (let j = 0; j < rows.length; j++) {
                    const json = {};
                    json['roles'] = "SELLER";
                    json['password'] = bcrypt.hashSync(rows[j][3]+'@123', saltRounds);
                    ...
                    if (!json.email)
                        return requiredField(res, 'Email', json.email);

                    if (!json.username)
                        return requiredField(res, 'User Name', json.username);

                    if (!json.mobile_number)
                        return requiredField(res, 'Phone', json.mobile_number);
                ...
                UserLogins.insertMany(productArray).then(function () {
    ...
    forgotpassword: async (req, res, next) => {
        try {
            ...
            let isUser = await UserLogins.findOne({ $or: [{ 'email': new RegExp(`^${username}$`, 'i') }] })
...
if(old_password === undefined){
    ...
    const comparePass =  await bcrypt.compare(password, current_password);
    ...
    if(comparePass) {
        return res.send({ status: false, message: 'Please enter diffrent password then old password' });    
    }
    ...
    updateprofile: async (req, res, next) => {
        ...
        Profile.findOne({ loginid: req.user._id }).then((data) => {
            ...
            if (data && data._id) {
                ...
                Profile.update({ loginid: req.user._id }, req.body ).then((data) => {
                    res.send({ status: true, data })
                    return;
                }).catch((err) => {
                ...
                UserLogins.update({ _id: req.user._id }, {username :name } ).then((data) => {
                    res.send({ status: true, data })
                    return;
                }).catch((err) => {
                    res.send({ status: false, message: err.errmsg })
    ...
    approvebussiness: async (req, res, next) => {
        ...
        try {
            ...
            let msg_approve = (verify ? `Admin has been approved your business verification.` : 
            `Admin has been disapprove your business verification.`);

            console.log("verify_________" , verify)
            console.log("msg_body_________" , msg_body)



            Helper.sendEmail(isUser.email, 'Bussiness Verify Status', msg_body);

            await SendNotification('', user_id, 'approve_business_verify',msg_approve );
    ...
    getprofile: async (req, res, next) => {
        ...
        try {
            ...
            const profile = await Profile.findOne({ loginid: profileId }).exec();


                profile.email = userLogin.email
    ...
    createaddress: (req, res, next) => {
        ...
        // return false
        let isEmpty = notEmpty({  phone, fname, lname,  add1, state, postal })
    ...
    collections: (req, res, next) => {
        ...
        //     },
        // ]);
        Category.aggregate([{ $lookup: { 'from': 'products', "localField": "_id", "foreignField": "category", as: 'products' } }
        ,
        ...
        let data = d.map((item,index) => {
            ...
            item.products.category_name = item.name;      
            return item;
        });
...
let orderDetail = await Order.find({ "product.id": id, loginid:user_id });
...
console.log("orderDetail" , orderDetail)
                ...
                let products = await Product.findOne({ _id: req.body.id }).populate('loginid','email mobile_number username').lean().exec();
                let seller_address = await Profile.findOne({ loginid: products.loginid._id });
                if(seller_address){
                    products.seller_address = seller_address;
                }

                if (products) {
                    let trending_count = 0;
                    if(products.top_trending){
                        trending_count =  products.top_trending + 1;
                    }else{
                        trending_count = 1;   
                    }
                  await Product.updateOne({ _id: req.body.id }, { $set: { top_trending: trending_count } });
                    ...
                    let sub_cate = await SubCategoryModel.findOne({ _id: products.subCategory })
                    ...
                    check.isProductBuy = isProductBuy
                    check.sub_cat_name = sub_cate.name
    ...
    contactUsList: async (req, res, next) => {
        try {
         ...
         return res.send({ status: true, data: data.reverse(),  message: 'Contact List get successfully' });
...
function requiredField(res, column, sku) {
    if (sku) {
        ...
        return res.send({ status: false, message: `${column} is required for insert User` });
    }
}
