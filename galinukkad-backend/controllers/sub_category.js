...
module.exports = {
    createSubCategory: async (req, res, next) => {
        ...
        const { name, slug, description, parent_category, attributes } = req.body;
    ...
    updateSubCategory: (req, res, next) => {
        ...
        const { name, slug, description, _id, parent_category, attributes } = req.body;
        if (!slug || !name || !_id) {
            res.send({ status: false, message: "Required Parameter is missing" });
            return;
        }
    ...
    getSubCategoryaAll: async (req, res, next) => {
        ...
        if(parent_category_id !== 'all' && parent_category_id !== undefined) {
            queries = { parent_category: parent_category_id }
        }
        // console.log("reqBody______________" , queries);

        Sub_Category.find(queries ? queries : "").populate('parent_category', '_id name slug description')
